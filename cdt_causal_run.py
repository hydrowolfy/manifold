...
class Causal:
    ...
    def add_tet(self, tet):
        ...
        for e in itertools.combinations(tet, 2):
            ...
            self.ecnt[f] = c
            if c == 1 and self.time[e[0]] == self.time[e[1]]:
                self.sedges.add(f)
    ...
    # -------------------------------------------------- Metropolis
    def attempt(self, rng, k0, k3, eps, V, k22=0.0):
    ...
    def sweep(self, rng, k0, k3, eps, V, k22=0.0):
        for _ in range(max(1, self.N3)):
            self.attempt(rng, k0, k3, eps, V, k22)
...
def run_chunk(a):
    ...
    rec = dict(k0=a.k0, T=a.T, V=a.V, eps=a.eps, seed=a.seed, k3=round(k3, 4), sweeps=done,
               tuned=done >= a.tune, k22=a.k22, wall=round(_time.time() - t0, 1),
