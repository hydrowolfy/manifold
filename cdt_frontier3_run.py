#!/usr/bin/env python3
"""cdt_frontier3_run.py -- STAGE 2 (exotic) frontier: cdt_frontier2_run.py core VERBATIM
(itself cdt_causal_run.py core + the campaign-7 hub term) plus two NON-LOCAL action/measure
terms that see GLOBAL structure (what campaigns 6/7 -- per-simplex k0/k22 and a per-vertex hub
cap -- cannot):
  NL-P  lam_p * sum_t (n(t) - nbar)^2   n(t)=#spatial triangles in slice t, nbar=N_stris/T.
        NON-SEPARABLE across the foliation (nbar couples every slice): minimized only by a
        GLOBALLY uniform spatial-volume profile -> penalizes condensation / re-weights toward
        uniform slices. No local per-simplex weight can express this.
  NL-D  lam_d * sum_v (deg_v - D0d)^2   symmetric global degree-distribution counter-term,
        D0d = flat-torus degree 14. Penalizes hubs AND under-connected vertices, driving the
        FULL degree distribution to the flat delta -- strictly stronger than campaign 7's
        one-sided cap sigma*sum max(0,deg-D0)^2. Offsets the entropic hub preference.
Goal: test whether a non-local term can SELECT the flat joint 3-manifold that provably EXISTS in
the ensemble (the exact flat calibrant == benchmark) but is entropically unselected -- pulling
BOTH d_H and d_s onto benchmark with a UNIFORM (low-CV) profile. Detailed balance: both terms are
state functions; proposals are unchanged, so Metropolis with exp(-dS_total) preserves DB (the
per-move deltas are verified against brute force for all 5 moves in --selftest). Original
docstring follows.
...
class Causal:
    def __init__(self, T):
        ...
        self.sprof = Counter()    # time slice -> #spatial triangles (profile n(t)); maintained incrementally
    ...
    def add_tet(self, tet):
        ...
        for tri in itertools.combinations(tet, 3):
            ...
            if len(s) == 1 and len({self.time[v] for v in tri}) == 1:
                self.stris.add(f)
                self.sprof[self.time[next(iter(f))]] += 1
    ...
    def _degdev_delta(self, rems, adds, D0):
        # STAGE 2 NL-D: net change in sum_{alive v} (deg_v - D0)^2 (SYMMETRIC full-distribution
        # counter-term). Uses current self.ecnt/self.deg (call BEFORE mutating). Alive iff deg>0
        # (== in valive): a newly created vertex has no 'before' term, a deleted one no 'after'.
        cnt = Counter()
        for t in rems:
            for e in itertools.combinations(t, 2):
                cnt[FS(e)] -= 1
        ...
        for v, d in dchg.items():
            old = self.deg.get(v, 0)
            new = old + d
            po = (old - D0) * (old - D0) if old > 0 else 0.0
            pn = (new - D0) * (new - D0) if new > 0 else 0.0
            s += pn - po
        return s

    def _profvar_delta(self, rems, adds, lam_p):
        # STAGE 2 NL-P: net change in lam_p * sum_t (n(t) - nbar)^2, n(t)=#spatial triangles in
        # slice t, nbar = N_stris/T. Non-local (nbar couples all slices). Only (2,6)/(6,2) change
        # N_stris and only in one slice; (2,3)/(3,2)/(4,4) leave the profile fixed. Uses current
        # self.sprof and len(self.stris). Call BEFORE mutating.
        cnt = Counter()
        for t in rems:
        ...
        for f, dc in cnt.items():
            if dc == 0:
                continue
            ts = {self.time[v] for v in f if v in self.time}  # new 2-6 vertex has no time yet; >=2 existing verts fix the slice
            if len(ts) != 1:
                continue
            before = len(self.tri2.get(f, ()))
            after = before + dc
            be = 1 if before > 0 else 0
            af = 1 if after > 0 else 0
            if af != be:
                dslice[next(iter(ts))] += (af - be)
        if not dslice:
            return 0.0
        T = self.T
        Ns = len(self.stris)
        dNs = sum(dslice.values())
        dQ = 0.0
        for t, d in dslice.items():
            n = self.sprof.get(t, 0)
            dQ += (n + d) * (n + d) - n * n
        dcouple = ((Ns + dNs) * (Ns + dNs) - Ns * Ns) / T
        return lam_p * (dQ - dcouple)

    # -------------------------------------------------- Metropolis
    def attempt(self, rng, k0, k3, eps, V, k22=0.0, sigma=0.0, D0=18.0, lam_d=0.0, D0d=14.0, lam_p=0.0):
        ...
        if lam_d != 0.0:
            dS += lam_d * self._degdev_delta(rems, adds, D0d)
        if lam_p != 0.0:
            dS += self._profvar_delta(rems, adds, lam_p)
    ...
    def sweep(self, rng, k0, k3, eps, V, k22=0.0, sigma=0.0, D0=18.0, lam_d=0.0, D0d=14.0, lam_p=0.0):
        for _ in range(max(1, self.N3)):
            self.attempt(rng, k0, k3, eps, V, k22, sigma, D0, lam_d, D0d, lam_p)
...
def validate(st, heavy=False):
    ...
    sprofc = Counter()
    for f in stris:
        sprofc[st.time[next(iter(f))]] += 1
    if hasattr(st, "sprof"):
        assert {k: v for k, v in st.sprof.items() if v} == {k: v for k, v in sprofc.items() if v}, "sprof cache drift"
...
def selftest():
    ...
    # STAGE 2: verify both non-local term deltas match brute force over every move type (DB safety)
    def _bf_profvar(stx):
        pf = Counter()
        for ff in stx.stris:
            pf[stx.time[next(iter(ff))]] += 1
        nb = sum(pf.values()) / stx.T
        return sum((pf.get(tt, 0) - nb) ** 2 for tt in range(stx.T))

    def _bf_degdev(stx, D0):
        return sum((d - D0) ** 2 for d in stx.deg.values() if d > 0)

    stv = seed_state(5); rngv = random.Random(7)
    for _ in range(120):
        stv.sweep(rngv, k0=1.0, k3=0.6, eps=0.02, V=220)
    validate(stv, heavy=False)
    per_key = Counter(); ok = 0
    for _ in range(60000):
        key = ("23", "32", "26", "62", "44")[rngv.randrange(5)]
        p = getattr(stv, "prop_" + key)(rngv)
        if p is None:
            continue
        rems, adds = p[3], p[4]
        pv_pred = stv._profvar_delta(rems, adds, 1.0)
        dd_pred = stv._degdev_delta(rems, adds, 14.0)
        pv0 = _bf_profvar(stv); dd0 = _bf_degdev(stv, 14.0)
        if len(p) == 6 and p[5][0] == "newv":
            stv.time[p[5][1]] = p[5][2]; stv.next_id += 1
        for tet in rems:
            stv.rem_tet(tet)
        for tet in adds:
            stv.add_tet(tet)
        pv1 = _bf_profvar(stv); dd1 = _bf_degdev(stv, 14.0)
        assert abs((pv1 - pv0) - pv_pred) < 1e-6, "NL-P delta mismatch [%s]: bf=%r pred=%r" % (key, pv1 - pv0, pv_pred)
        assert abs((dd1 - dd0) - dd_pred) < 1e-6, "NL-D delta mismatch [%s]: bf=%r pred=%r" % (key, dd1 - dd0, dd_pred)
        per_key[key] += 1; ok += 1
        if ok >= 2500 and all(per_key[k] > 0 for k in ("23", "32", "26", "62", "44")):
            break
    validate(stv, heavy=True)
    assert all(per_key[k] > 0 for k in ("23", "32", "26", "62", "44")), "a move type never tested: %s" % dict(per_key)
    # reverse-delta antisymmetry (round-trip 2-3 then 3-2): forward + reverse cancel exactly
    stv2 = seed_state(4); rngv2 = random.Random(11)
    p23 = None
    for _ in range(4000):
        p23 = stv2.prop_23(rngv2)
        if p23 is not None:
            break
    d_fwd = stv2._degdev_delta(p23[3], p23[4], 14.0); pv_fwd = stv2._profvar_delta(p23[3], p23[4], 1.0)
    for tet in p23[3]:
        stv2.rem_tet(tet)
    for tet in p23[4]:
        stv2.add_tet(tet)
    d_rev = stv2._degdev_delta(p23[4], p23[3], 14.0); pv_rev = stv2._profvar_delta(p23[4], p23[3], 1.0)
    assert abs(d_fwd + d_rev) < 1e-6 and abs(pv_fwd + pv_rev) < 1e-6, "reverse delta not antisymmetric"
    print("  non-local term deltas verified vs brute force over %d moves %s; reverse antisymmetric" % (ok, dict(per_key)))
...
def run_chunk(a):
    ...
    tag = "k0%+.2f_T%d_V%d_s%d_sig%.3f_ld%.4f_lp%.4f_k22%.2f" % (a.k0, a.T, a.V, a.seed, a.sigma, a.lam_d, a.lam_p, a.k22)
    ...
    if os.path.exists(sf):
        ...
        if not getattr(st, "sprof", None):  # resume pickle lacking the profile cache: rebuild from stris
            st.sprof = Counter()
            for _f in st.stris:
                st.sprof[st.time[next(iter(_f))]] += 1
    else:
    ...
    while done < a.sweeps and _time.time() - t0 < a.budget_s:
        st.sweep(rng, a.k0, k3, a.eps, a.V, a.k22, a.sigma, a.D0, a.lam_d, a.D0d, a.lam_p)
        if done < a.tune:
    ...
    rec = dict(k0=a.k0, T=a.T, V=a.V, eps=a.eps, seed=a.seed, k3=round(k3, 4), sweeps=done,
               tuned=done >= a.tune, k22=a.k22, sigma=a.sigma, D0=a.D0, lam_d=a.lam_d, D0d=a.D0d, lam_p=a.lam_p,
...
def main():
    ...
    ap.add_argument("--lam-d", type=float, default=0.0, dest="lam_d", help="STAGE2 NL-D: symmetric global degree counter-term coeff on sum_v (deg-D0d)^2")
    ap.add_argument("--D0d", type=float, default=14.0, help="STAGE2 NL-D target degree (flat-torus degree 14)")
    ap.add_argument("--lam-p", type=float, default=0.0, dest="lam_p", help="STAGE2 NL-P: non-local profile-uniformity coeff on sum_t (n(t)-nbar)^2")
